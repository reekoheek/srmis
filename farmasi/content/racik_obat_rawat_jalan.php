...
<?php
	...
	$id_p = $_GET['pasien_id'];
	$qar = mysql_query("SELECT * FROM pasien WHERE id='$id_p'");
		...
		<td id="tengah_isi" >
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<form method="post" action="home.php?hal=action/insert_nama_racik_rawat_jalan&no_racik=<? ?>&no_resep=<?=no_resep?>&pasien_id=<?=$id_p?>&param_no=<?=$param_no?>&nama=<?=$nama?> " enctype="multipart/form-data">
					<table border="0" cellpadding="2" cellspacing="2" width="100%">
						...
						<tr>
							...
							?>
								...
								<form method="post" action="home.php?hal=action/simpan_racik_obat_rawat_jalan" enctype="multipart/form-data">
									<input type="hidden" readonly="true" value="<?=$_GET['sub_margin']?>" name="sub_margin">
									<input type="hidden" readonly="true" value="<?=$_GET['no_racik']?>" name="no_racik">
									<input type="hidden" readonly="true" value="<?=$_GET['no_resep']?>" name="no_resep">
									<input type="hidden" readonly="true" value="<?=$_GET['param_no']?>" name="param_no">
								...
								<?php
									...
									while ($r = mysql_fetch_array($q))
									{
										...
										echo "</td>
											...
											<a href=\"home.php?hal=action/hapus_racik_obat_rawat_jalan&fld02=$fld02&qty=$r[qty]&no_racik=$no_racik&id=$r[id]&pasien_id=$pasien_id&kd_barang=$r[kode_obat]&diberi=$r[diberi]&no_resep=$no_resep&param_no=$param_no&sub_margin=$_GET[sub_margin]\" 
