...
<?php
	...
	if ($_GET['hehe'])
	{
		$q=mysql_query("SELECT * FROM permintaan_unitdetail WHERE No_SPP='$No_SPP' AND status_detail = '2'");
		$r=mysql_fetch_array($q);
		
		//echo $q;
		if (!$r)
		{
			$qp=("UPDATE permintaan_unit SET status = '7' WHERE No_SPP='$No_SPP'");
			execSQL($qp);
			echo "masuk update";
		}
		...
		if ($_POST['in']==1)
	{
		...
		//echo "dapet";
		$hehe=1;
		print "<script>alert('Data Berhasil di Input');location.href='input_buat_retur_rawat_inap.php?No_SPP=$No_SPP&hehe=$hehe'</script>";
...
<form method="post" name="form" action="input_buat_retur_rawat_inap.php?No_SPP=<?=$rar2['No_SPP']?>&id=<?=$rar2['id']?>&in=1">
				...
				<tr>
					...
					<tr>
						<td align="left" width="100">No BTB </td>
							<td><input type="text" size="20" name="no_BTB" value="<?= $rar['No_BTB']?>" style="background-color:#CCCFFF; " readonly="true">
