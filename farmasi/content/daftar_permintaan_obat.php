				...
				<tr>
					...
					<table border="0" cellpadding="2" cellspacing="2" width="100%">
						<tr>
							<td align="right">
								<div id="container">
								<form method="post" action="home.php?hal=content/daftar_permintaan_obat" enctype="multipart/form-data">
						...
						<tr>
							<td>
								<?php
									...
									if ($cari)
									{
									$query  = mysql_query ("SELECT * FROM ms_barang WHERE  flags <> '9' AND stok <> '0' AND 
									kd_barang LIKE '$cari%' ORDER BY ex_year,ex_month,ex_date ASC ");
									}
									else
									{
									$query  = mysql_query ("SELECT * FROM ms_barang WHERE  flags<>'9' AND stok <> '0' ORDER BY ex_year,ex_month,ex_date ASC
									...
									echo '<table cellpadding=2 cellspacing=2 width=100% style="border:1px  solid  #CCCCCC; ">
											<tr bgcolor=#414141 align=center>
												...
												<td><font color=#FFFFFF>Nama Obat</font></td>
													<td><font color=#FFFFFF>Expired</font></td>
									...
									while ($result = mysql_fetch_array($query))
									{
										...
										echo "<td width=70px>$result[kd_barang]</td>
											<td>$result[nama]</td>
											<td align=center>$result[expire_date]</td>
											<td align=right>";
											rupiah($result[harga_dosp]);
											echo "</td>
											<td align=right>$result[stok]</td>
											<td align=center width=60px>
											<form method=post action='home.php?hal=content/input_permintaan_obat' enctype=multipart/form-data>
									...
									{
   										$page = $pageNum - 1;
   								   	 	$prev = " <a href=\"$self?page=$page&hal=content/daftar_permintaan_obat&no_SPP=$no_SPP&tgl=$tgl&id=$id\"><font color='#565957' size=-1>[&laquo;]</font></a> ";

    									$first = " <a href=\"$self?page=1&hal=content/daftar_permintaan_obat&no_SPP=$no_SPP&tgl=$tgl&id=$id\"><font color='#565957' size=-1>[&laquo;&laquo;]</font></a> ";
									}
									else
									{
   			 							$prev  = ' [&laquo;] ';
									...
									{
    									$page = $pageNum + 1;
    									$next = " <a href=\"$self?page=$page&hal=content/daftar_permintaan_obat&no_SPP=$no_SPP&tgl=$tgl&id=$id\"><font color='#565957' size=-1>[&raquo;]</font></a> ";

    									$last = " <a href=\"$self?page=$maxPage&hal=content/daftar_permintaan_obat&no_SPP=$no_SPP&tgl=$tgl&id=$id\"><font color='#565957' size=-1>[&raquo;&raquo;]</font></a> ";
									}
									else
									{
   										$next = ' [&raquo;] ';
