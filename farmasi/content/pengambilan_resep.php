...
//}					
	...
	$qhd=mysql_query("select * from resep_head where no_resep='$no_resep'");
	...
	if (isset($rhd['sub_margin']))
	{
		...
		$tampil=$margin;
		$namamargin=$rmar2['sub_klasifikasi'];
	}
	else
	{
		$margin=30/100;
				...
				<div id="container">			<? if(!$_GET['id_pasien']){ ?> <input type="text" align="center" name="no_rm" size="18" value="-" readonly="true"><? }else{ ?>
					  ...
					  <tr>
						  ...
						  <td><?php
							...
							{
							?>
                            <input type="text" name="jenis" value="<? echo $re['cara_bayar']." ".$namamargin ?>" readonly="true" size="30" style="background-color:#CCCFFF;" readonly="true">
                            <?php
							}
							else
							{
