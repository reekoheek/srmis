...
$param_no = $count;
	...
	if (!$re)
	{
		...
		$r = mysql_query($q);
		print "<script>location.href='home.php?hal=content/Permintaan_Obat_rawat_jalan&No_SPP=$no_SPP&Tgl_SPP=$date'</script>";
	}
	else
	{
		...
		$re2=mysql_fetch_array($qe2);
				$no=$re['No_SPP'];
				$tgl=$re['Tgl_SPP'];
				print "<script>location.href='home.php?hal=content/Permintaan_Obat_rawat_jalan&No_SPP=$no&Tgl_SPP=$tgl' </script>";
		}
		else
		{
				$q = "INSERT INTO permintaan_unit (No_SPP, param_no, tgl_SPP,UsrBuat,Unit) 
