<?php
	...
	while ($r=mysql_fetch_array($q))
	{
		//$q_ms = mysql_query ("SELECT * FROM ms_barang WHERE kd_barang = '$r[barang_id]'");
		//$r_ms = mysql_fetch_array ($q_ms);
		
		$q_amc = mysql_query ("SELECT * FROM amc2 WHERE barang_id = '$r[barang_id]' AND tahun = '$thn'");
		$r_amc = mysql_fetch_array ($q_amc);
		
		//echo $r_amc['barang_id']."<br>";
