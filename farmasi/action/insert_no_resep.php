...
$q2=mysql_query("SELECT * FROM simrs.pasien,simrs.kunjungan,simrs.kunjungan_kamar,simrs.kamar,simrs.pelayanan 
				 WHERE pasien.id=kunjungan.pasien_id AND 
				 	   kunjungan.id = kunjungan_kamar.kunjungan_id AND
				 	   kunjungan_kamar.kamar_id = kamar.id AND
				 	   kamar.pelayanan_id = pelayanan.id AND
				 	   LAST_INSERT_ID(kunjungan.kunjungan_ke) AND 
					   pasien.id='$id' ORDER BY kunjungan.kunjungan_ke DESC LIMIT 1");
$r2=mysql_fetch_array($q2);
...
elseif($cara_masuk=="RUANG TINDAKAN")
{
	$kd="OKA/";
}
