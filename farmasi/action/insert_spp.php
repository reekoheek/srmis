...
$param_no = $count;
	...
	if (!$re)
	{
		...
		$q = "INSERT INTO permintaan_unit (No_SPP, param_no, tgl_SPP,UsrBuat,Unit,create_datetime,create_date) 
				  VALUES ('$no_SPP','$param_no', '$date','".$_SESSION['U_USER']."','".$_SESSION['U_SUBUNIT']."', now(), '$tanggal')";
		$r = mysql_query($q);
		print "<script>location.href='home.php?hal=content/Permintaan_Obat&No_SPP=$no_SPP&Tgl_SPP=$date'</script>";
	}
	else
	{
		...
		$re2=mysql_fetch_array($qe2);
				$no=$re['No_SPP'];
				$tgl=$re['Tgl_SPP'];
				print "<script>location.href='home.php?hal=content/Permintaan_Obat&No_SPP=$no&Tgl_SPP=$tgl' </script>";
		}
		else
		{
				$q = "INSERT INTO permintaan_unit (No_SPP, param_no, tgl_SPP,UsrBuat,Unit,create_datetime,create_date) 
