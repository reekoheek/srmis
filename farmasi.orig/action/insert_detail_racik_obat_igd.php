<?php
	...
	if ($_POST['nama_obat'] == "")
	{
		print "<script>alert('Nama Obat Harus Di Isi Terlebih Dahulu.');location.href='../content/input_racik_obat_igd.php?fld02=$fld02&no_racik=$no_racik&id=$pasien_id&ket_$ket&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama=$nama&sub_margin=$sub_margin'</script>";
	}
	
	else if ($_POST['jumlah'] == "")
	{
		print "<script>alert('Keterangan Harus Di Isi Terlebih Dahulu.');location.href='../content/input_racik_obat_igd.php?fld02=$fld02&no_racik=$no_racik&id=$pasien_id&ket_$ket&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama=$nama&sub_margin=$sub_margin'</script>";
	}
	else
	{
	$kode_obat = $_POST['kd_obatt'];
	...
	if ($_POST['jumlah'] > $stok_ms)
	{
		...
		location.href='../content/input_racik_obat_igd.php?fld02=$fld02&no_racik=$no_racik&id=$pasien_id&ket_$ket&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama=$nama&sub_margin=$sub_margin'</script>";
	}
	else
	{
	//ini baru rumus
