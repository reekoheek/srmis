<?php
	...
	if ($_POST['nama_obat'] == "")
	{
		print "<script>alert('Nama Obat Harus Di Isi Terlebih Dahulu.');location.href='../content/input_racik_obat_umum.php?no_racik=$no_racik&ket=$ket&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama_pas=$nama_pas&rs_asal=$rs_asal&kd_barang=$kode_obat&biaya_racik=$biaya_racik&sub_margin=$sub_margin&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	else if ($_POST['ket'] == "")
	{
		print "<script>alert('Keterangan Harus Di Isi Terlebih Dahulu.');location.href='../content/input_racik_obat_umum.php?no_racik=$no_racik&ket=$ket&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama_pas=$nama_pas&rs_asal=$rs_asal&kd_barang=$kode_obat&biaya_racik=$biaya_racik&sub_margin=$sub_margin&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	...
	if ($_POST['jumlah'] > $r1k['stok'])
	{
		...
		location.href='../content/input_racik_obat_umum.php?no_racik=$no_racik&ket=$ket&param_no=$param_no&no_resep=$no_resep&nama_racikan=$nama_racikan&nama_pas=$nama_pas&rs_asal=$rs_asal&kd_barang=$kode_obat&biaya_racik=$biaya_racik&sub_margin=$sub_margin&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	else
	{
	//ini baru rumus
