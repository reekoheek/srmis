...
<script>
	function lookup(inputString) {
		if(inputString.length == 0) {
			...
			$.post("action/string_obat_rawat_inap.php", {mysearchString: ""+inputString+""}, function(data){
				if(data.length >0) {
					$('#suggestions').show();
				...
				<tr>
					...
					<table border="0" cellpadding="2" cellspacing="2" width="100%">
						<tr>
							<td align="right">
								<div id="container">
								<form method="post" action="home.php?hal=content/daftar_racik_obat_rawat_inap" enctype="multipart/form-data">
								...
								<?php
									...
									while ($result = mysql_fetch_array($query))
									{
										...
										echo "<td width=70px>$result[kd_barang]</td>
											...
											<form method=post action='home.php?hal=content/input_racik_obat_rawat_inap' enctype=multipart/form-data>
									...
									{
   										$page = $pageNum - 1;
   								   	 	$prev = " <a href=\"$self?page=$page&hal=content/daftar_racik_obat_rawat_inap&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&laquo;]</font></a> ";

    									$first = " <a href=\"$self?page=1&hal=content/daftar_racik_obat_rawat_inap&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&laquo;&laquo;]</font></a> ";
									}
									else
									{
   			 							$prev  = ' [&laquo;] ';
									...
									{
    									$page = $pageNum + 1;
    									$next = " <a href=\"$self?page=$page&hal=content/daftar_racik_obat_rawat_inap&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&raquo;]</font></a> ";

    									$last = " <a href=\"$self?page=$maxPage&hal=content/daftar_racik_obat_rawat_inap&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&raquo;&raquo;]</font></a> ";
									}
									else
									{
   										$next = ' [&raquo;] ';
