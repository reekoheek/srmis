...
<script>
	function lookup(inputString) {
		if(inputString.length == 0) {
			...
			$.post("action/string_spp_igd.php", {mysearchString: ""+inputString+""}, function(data){
				if(data.length >0) {
					$('#suggestions').show();
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<table border="0" cellpadding="2" cellspacing="2" width="100%">
						<tr>
							<form method="post" action="home.php?hal=content/list_spp_igd" enctype="multipart/form-data">
							<td align="right">
						  <!--
							...
							echo '<table border=0 cellpadding=2 cellspacing=2 width=100%>
									...
									if (!empty($result2['No_BPB']))
									{
										...
										echo "<td align=center width=80px>$result2[tgl_bpb]</td>
											...
											{
												echo "<td align=center><a href='home.php?hal=action/insert_btb_igd&No_SPP=$result2[No_SPP]&UsrBuat=".$_SESSION['U_USER']."' style=visibility:hidden>Terima Barang</a></td>";
											}
											elseif (($result2['status']==2 or $result2['status']==8 ) AND ($result2['Unit'] == $_SESSION['U_SUBUNIT']))
											{
												echo "<td align=center><a href='home.php?hal=action/insert_btb_igd&No_SPP=$result2[No_SPP]&UsrBuat=".$_SESSION['U_USER']."'>Terima Barang</a></td>";
											}
											else
											{
												echo "<td align=center><a href='home.php?hal=action/insert_btb_igd&No_SPP=$result2[No_SPP]&UsrBuat=".$_SESSION['U_USER']."' style=visibility:hidden>Terima Barang</a></td>";
