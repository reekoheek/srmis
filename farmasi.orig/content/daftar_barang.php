				...
				<tr>
					...
					<tr valign="top">
						...
						<?php
							...
							if ($cari)
							{
								$query_rec  = mysql_query ("SELECT DISTINCT(kd_barang) FROM ms_barang WHERE kd_barang LIKE '$cari%' AND flags <> '9' ORDER BY ex_year,ex_month,ex_date ASC");
							}
							else
							{
								$query_rec  = mysql_query ("SELECT DISTINCT(kd_barang) FROM ms_barang WHERE flags <> '9' ORDER BY ex_year,ex_month,ex_date ASC");
							}
							$rec=0;
						...
						<tr>
							<td>
								<?php
									...
									if ($cari)
									{
									$query  = mysql_query ("SELECT DISTINCT(kd_barang) FROM ms_barang WHERE kd_barang LIKE '$cari%' AND flags <> '9' ORDER BY ex_year,ex_month,ex_date ASC");
									}
									else
									{
									
									$query  = mysql_query ("SELECT DISTINCT(kd_barang)FROM ms_barang WHERE flags <> '9' ORDER BY ex_year,ex_month,ex_date ASC
									...
									while ($result = mysql_fetch_array($query))
									{
										...
										$q_ms = mysql_query ("SELECT * FROM ms_barang WHERE kd_barang = '$result[kd_barang]'");
										$r_ms = mysql_fetch_array ($q_ms);
										
										$q_stok = mysql_query ("SELECT stok,kd_barang FROM ms_barang WHERE kd_barang = '$result[kd_barang]'");
										$stok = 0;
										...
										echo "<td width=70px>$r_ms[kd_barang]</td>
											<td width=180px>$r_ms[nama]</td>";
											if ($r_ms['stok'] <= $r_ms['stok_min'])
											{
												echo "<td width=60px align=right><font color=blue>$stok</font></td>";
											}
											else
											{
												echo "<td width=60px align=right>$stok</td>";
											}
											echo "<td align=right width=80px>";
											rupiah($r_ms[harga_dosp]);
											echo "</td>";
											if (($pmonth == $r_ms['ex_month']) AND ($pyear == $r_ms['ex_year']))
											{ 
												echo "<td width=70px align=center><font color=blue>$r_ms[expire_date]</font></td>";
											}
											else if ((($ppmonth > $r_ms['ex_month']) AND ($pyear >= $r_ms['ex_year']) AND ($pdate > $r_ms['ex_date'])) OR 
											(($ppmonth > $r_ms['ex_month']) AND ($pyear >= $r_ms['ex_year'])))
											{
												//$qy = mysql_query("UPDATE ms_barang SET flags='9', status='Non-Aktif' WHERE id='$result[id]'"); 
												echo "<td width=70px align=center><font color=red>$r_ms[expire_date]</font></td>";
											}
											else if (($ppmonth == $r_ms['ex_month']) AND ($pyear == $r_ms['ex_year']))
											{
												echo "<td width=70px align=center><font color=red>$r_ms[expire_date]</font></td>";
											}
											else
											{
											 	echo "<td width=70px align=center>$r_ms[expire_date]</td>";
											}
											echo "<td align=center width=200px>
											<a href=home.php?hal=content/add_daftar_barang&id=$r_ms[id]>
											<font style=font-size:12px;>ADD STOCK</font></a> | 
											<a href=home.php?hal=content/input_daftar_barang&id=$r_ms[id]>
											<font style=font-size:12px;>EDIT</font></a> | 
											<a href=\"home.php?hal=action/hapus_barang&id=$r_ms[id]\" 
											onClick=\"return confirm('Apakah Anda benar-benar akan menghapus $r_ms[nama] ?')\">
											<font style=font-size:12px;>HAPUS</font></a> |
											<a href=home.php?hal=content/detail_barang&barang_id=$r_ms[id]><font style=font-size:12px;>DETAIL</font></a>
											</td>
											</tr>";
									...
									$query   = "SELECT COUNT(DISTINCT(kd_barang)) AS numrows FROM ms_barang WHERE flags<>'9' ORDER BY ex_year,ex_month,ex_date ASC";
