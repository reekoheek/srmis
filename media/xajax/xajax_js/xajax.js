...
sXml+="</q></xjxquery>";return sXml;}
	this.getFormValuesExcept = function(frm)
	{
		var objForm;
		...
		if (objForm && objForm.tagName == 'FORM')
		{
			...
			for( var i=0; i < formElements.length; i++)
			{
				...
				if (formElements[i].type && (formElements[i].type == 'radio' || formElements[i].type == 'checkbox') && formElements[i].checked == false)
					continue;
				if (formElements[i].type && (formElements[i].type == 'button' || formElements[i].type == 'reset'))
					continue;
...
r.onreadystatechange=function(){if(r.readyState!=4)
	{if(tantosShowLoading) show_loading();return;}if(r.status==200){if(xajaxDebug)xajax.DebugMessage("Received:\n"+r.responseText);if(r.responseXML&&r.responseXML.documentElement)
		{if(tantosShowLoading) remove_loading();xajax.processResponse(r.responseXML);}else{var errorString="Error: the XML response that was returned from the server is invalid.";errorString+="\nReceived:\n"+r.responseText;trimmedResponseText=r.responseText.replace(/^\s+/g,"");trimmedResponseText=trimmedResponseText.replace(/\s+$/g,"");if(trimmedResponseText!=r.responseText)
