...
function xcari_specimen(evt) {
    evt = (evt) ? evt : event;
    var charCode = (evt.charCode) ? evt.charCode : ((evt.which) ? evt.which : evt.keyCode);
    if (charCode == 13 || charCode == 9) { //enter tab
		xajax_cari_specimen('0',xajax.getFormValues('cari_specimen'));
	}
    return true;
}
...
function tutup_specimen() {
	enable_kunjungan();
	document.getElementById('form_cari_specimen').style.display='none';
}
function buka_specimen(obj) {
	disable_kunjungan();
	var atas = (ajaxTooltip_getTopPos(obj) - 200);
	document.getElementById('form_cari_specimen').style.top = atas + 'px';
	document.getElementById('form_cari_specimen').style.display='';
	fokus('specimen');
}
...
function tutup_kunjungan() {
	...
	document.getElementById("tbody_input_specimen").innerHTML = '';
...
function cetak_kwitansi(id_kwitansi) {
	cetak(URL + 'lab/kwitansi_cetak/?id_kwitansi=' + id_kwitansi, 350, 500);
}
