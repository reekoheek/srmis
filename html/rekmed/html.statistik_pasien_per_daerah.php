...
<h3><?=$_TITLE;?></h3>
<div id="div_statistik_pasien_per_daerah">
<form method="post" action="<?$_SERVER['PHP_SELF']?>" name="statistik_pasien_per_daerah" id="statistik_pasien_per_daerah" onsubmit="return false;">
...
<table cellpadding="0" cellspacing="2" border="0" class="form">
	...
	<tr>
		...
		<td>
			...
			<select name="tgl_periksa_thn_end" id="tgl_periksa_thn_end" style="width: 70px;" onkeypress="focusNext( 'propinsi_id', event, 'tgl_periksa_bln_end', this)" class="inputan">
				<? for($i=$thn_start;$i<=$thn_skr;$i++) {
						if($i==$thn_skr) $sel = "selected";
						else $sel = "";
				?>
		...
		<td>
			<table cellpadding="0" cellspacing="4" border="0" class="form">
				<tr>
					<td>Propinsi</td>
					<td>
						<select name="propinsi_id" id="propinsi_id" style="width: 200px;" onkeypress="focusNext( 'kabupaten_id', event, 'tgl_periksa_thn_end', this)" class="inputan" onchange="xajax_ref_get_kabupaten('kabupaten_id', this.value)">
	...
	<tr>
		<td colspan="2" style="text-align: center;"><input type="button" name="cari" id="cari" value="Tampilkan" class="inputan" onclick="xajax_get_pasien_check(xajax.getFormValues('statistik_pasien_per_daerah'));" onkeypress="focusNext( 'jangka_waktu', event, 'desa_id', this)" /></td>
	</tr>
</table>
</form>
</div>
...
<div style="text-align:right;width:100%;">
	<img src="<?=IMAGES_URL?>printer.gif" alt="Cetak" onclick="cetak('<?=URL?>rekmed/statistik_pasien_per_daerah_cetak/');" class="printer_button" />
</div>
<h2 id="title"></h2>
...
Klik salah satu daerah untuk mendapatkan list pasien.
</div>
