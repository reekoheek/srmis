...
<table cellpadding="0" cellspacing="2" border="0" class="form">
	<tr>
		<td style="width: 150px;">Pelayanan</td><td>
			<select id="pelayanan_id" name="pelayanan_id" class="inputan" onkeypress="focusNext( 'tampilkan', event, 'cari', this)" style="width:200px">
				<option value="">-- Semua --</option>
				<? for($i=0;$i<sizeof($data_poli);$i++) :?>
					<option value="<?=$data_poli[$i][id]?>"><?=$data_poli[$i][nama]?></option>
				<? endfor; ?>
			</select>
		</td>
	</tr>
	...
	<tr>
		<td colspan="2" style="text-align: center;"><input type="button" name="cari" id="cari" value="Tampilkan" class="inputan" onclick="xajax_get_kunjungan_check(xajax.getFormValues('statistik_kunjungan'));" onkeypress="focusNext( 'pelayanan_id', event, 'tgl_periksa_thn_end', this)" /></td>
	</tr>
</table>
</form>
</div>
...
<img src="<?=IMAGES_URL?>printer.gif" alt="Cetak" onclick="cetak('<?=URL?>rekmed/statistik_kunjungan_ranap_cetak/', 1000);" class="printer_button" />
</div>
<h2 id="title"></h2>
