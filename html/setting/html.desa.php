...
<h3><?=$_TITLE?></h3>
<form method="post" action="<?$_SERVER['PHP_SELF']?>" name="form_desa" id="form_desa" onsubmit="return false;">
...
<table cellpadding="0" cellspacing="2" border="0" class="form">
	...
	<tr>
		<td>Kabupaten</td><td>
			<select name="kabupaten_id" id="kabupaten_id" class="inputan" onkeypress="focusNext( 'kecamatan_id', event, 'propinsi_id', this)" style="width:300px;" onchange="xajax_get_kecamatan(this.value);show_this_only();">
				<option value="">--- PILIH ---</option>
			</select>
			&nbsp;
	...
	<tr>
		<td>Kecamatan</td>
		<td>
			<select name="kecamatan_id" id="kecamatan_id" style="width: 300px;" onkeypress="focusNext( 'nama', event, 'kabupaten_id', this)" class="inputan" onchange="show_this_only();">
				<option value="">--- PILIH ---</option>
			</select>
			&nbsp;
			<label for="show_this_kec"><input type="checkbox" value="1" name="show_this_kec" id="show_this_kec" onclick="show_this_only();" />Tampilkan Hanya Kecamatan Ini</label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2" style="text-align: center;"><input type="button" name="simpan" id="simpan" value="Simpan" class="inputan" onclick="xajax_simpan_desa_check(xajax.getFormValues('form_desa'));" />&nbsp;&nbsp;
		<input type="reset" name="baru" value="Data Baru" class="inputan" onclick="xajax_reset_desa();" /></td>
	</tr>
</table>
</form>
<br />
