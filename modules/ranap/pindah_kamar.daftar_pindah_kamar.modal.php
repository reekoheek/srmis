...
Class Daftar_Ranap {
	...
	function buka_daftar_pindah_kamar($id_kunjungan_kamar) {
		...
		$objResponse->addClear("is_edit", "value");
	...
	function simpan_daftar_pindah_kamar($value) {
		$kon = new Konek;
		/*
		cek apakah user mendaftar pindah_kamar atau mengedit pindah_kamar
		*/
		if($value[is_edit] == "1") {
			// adalah pasien yang sudah dipindah, maka diedit
			$kon->sql = "
				UPDATE
					kunjungan_kamar
				SET
					...
					tgl_keluar = '".$value[tgl_keluar_thn]."-".$value[tgl_keluar_bln]."-".$value[tgl_keluar_tgl]." ".$value[tgl_keluar_jam].":".$value[tgl_keluar_mnt].":00'
				WHERE
					id = '".$value[id_kunjungan_kamar_stl_pindah]."'
			";
		} else {
			$kon->sql = "
				UPDATE 
			...
			VALUES (
				...
				'".$value[id_kunjungan]."',
				'".$value[kamar_id]."',
				'".$value[tgl_keluar_thn]."-".$value[tgl_keluar_bln]."-".$value[tgl_keluar_tgl]." ".$value[tgl_keluar_jam].":".$value[tgl_keluar_mnt].":00',
				'".$value[tgl_keluar_thn]."-".$value[tgl_keluar_bln]."-".$value[tgl_keluar_tgl]." ".$value[tgl_keluar_jam].":".$value[tgl_keluar_mnt].":00',
		...
		if($afek < 0) {
			...
			$objResponse->addScriptCall("xajax_tutup_daftar_pindah_kamar");
	...
	function tutup_daftar_pindah_kamar() {
		$objResponse = new xajaxResponse;
		$objResponse->addAssign("modal_daftar_pindah_kamar", "style.display", "none");
		$objResponse->addScript("document.getElementById('daftar_pindah_kamar').reset()");
	...
	function get_info_kamar($pelid, $kamarId) {
		...
		$tabel = new Table;
		$tabel->scroll = false;
		$tabel->addTh("No", "Kamar", "Jml TT", "Terisi", "Kosong");
...
$_xajax->registerFunction(array("buka_daftar_pindah_kamar", "Daftar_Ranap", "buka_daftar_pindah_kamar"));
$_xajax->registerFunction(array("buka_edit_pindah_kamar", "Daftar_Ranap", "buka_edit_pindah_kamar"));
$_xajax->registerFunction(array("simpan_daftar_pindah_kamar", "Daftar_Ranap", "simpan_daftar_pindah_kamar"));
$_xajax->registerFunction(array("simpan_daftar_pindah_kamar_check", "Daftar_Ranap", "simpan_daftar_pindah_kamar_check"));
$_xajax->registerFunction(array("tutup_daftar_pindah_kamar", "Daftar_Ranap", "tutup_daftar_pindah_kamar"));
