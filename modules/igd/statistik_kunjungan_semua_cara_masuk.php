...
Class Statistik_Kunjungan_Semua_Cara_Masuk {
	...
	function get_pasien($val) {
		...
		$title = "Statistik Kunjungan IRD Berdasar Cara Masuk";
		
		unset($_SESSION[igd][statistik_kunjungan_semua_cara_masuk]);
		...
		$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][title] = $title;
		$sql = "
			SELECT
		...
		for($i=0;$i<sizeof($data);$i++) {
			...
			$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][no][$i] = ($i+1);
			$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][nama][$i] = $data[$i][nama];
			$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][jml][$i] = $data[$i][jml];
			$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][total] = $total;
		$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][persen_total] = round($persen_total);
		$table->addRow("", "<b>Total</b>", $_SESSION[igd][statistik_kunjungan_semua_cara_masuk][total], $_SESSION[igd][statistik_kunjungan_semua_cara_masuk][persen_total]);
		$ret = $table->build();
		if(empty($_SESSION[igd][statistik_kunjungan_semua_cara_masuk][jml])) $_SESSION[igd][statistik_kunjungan_semua_cara_masuk][jml][0] = 1;
		if(empty($_SESSION[igd][statistik_kunjungan_semua_cara_masuk][nama]))	$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][nama][0] = "No Data";
		

		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[igd][statistik_kunjungan_semua_cara_masuk][title]));
		$_SESSION[igd][statistik_kunjungan_semua_cara_masuk][graph] = "<img src=\"" . URL . "igd/statistik_kunjungan_semua_cara_masuk_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Pasien\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[igd][statistik_kunjungan_semua_cara_masuk][graph]);
		return $objResponse;
	}
