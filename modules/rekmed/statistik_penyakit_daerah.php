<?
$_TITLE = "Statistik Penyakit Per Daerah";
Class Statistik_Penyakit {
	...
	function get_penyakit($val) {
		...
		if($val[tampilkan] != "Semua") {
			$batas = $val[tampilkan];
			$title = "Statistik ".$val[tampilkan]." Besar Penyakit";
		} else {
			$batas = 0;
			$title = "Statistik Penyakit";
		}

		unset($_SESSION[rekmed][statistik_penyakit_daerah]);
		...
		if($val[propinsi_id]) {
			...
			$kon->execute();
			$prop = $kon->getOne();
			$_SESSION[rekmed][statistik_penyakit_daerah][prop] = $prop[nama];
			...
			if($val[kabupaten_id]) {
				...
				$kon->execute();
				$kab = $kon->getOne();
				$_SESSION[rekmed][statistik_penyakit_daerah][kab] = $kab[nama];
				...
				if($val[kecamatan_id]) {
					...
					$kon->execute();
					$kec = $kon->getOne();
					$_SESSION[rekmed][statistik_penyakit_daerah][kec] = $kec[nama];
					...
					if($val[desa_id]) {
						...
						$kon->execute();
						$desa = $kon->getOne();
						$_SESSION[rekmed][statistik_penyakit_daerah][desa] = $desa[nama];
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$title .= "\nKelurahan " . $desa[nama];
					} else unset($_SESSION[rekmed][statistik_penyakit_daerah][desa]);
				} else unset($_SESSION[rekmed][statistik_penyakit_daerah][kec]);
			} else unset($_SESSION[rekmed][statistik_penyakit_daerah][kab]);
		} else unset($_SESSION[rekmed][statistik_penyakit_daerah][prop]);
		...
		$_SESSION[rekmed][statistik_penyakit_daerah][title] = $title;
		...
		for($i=0;$i<sizeof($new_data);$i++) {
			...
			$_SESSION[rekmed][statistik_penyakit_daerah][no][$i] = ($i+1);
			$_SESSION[rekmed][statistik_penyakit_daerah][kode][$i] = $new_data[$i][kode_icd];
			$_SESSION[rekmed][statistik_penyakit_daerah][nama][$i] = $new_data[$i][nama];
			$_SESSION[rekmed][statistik_penyakit_daerah][jml][$i] = $new_data[$i][jml];
			$_SESSION[rekmed][statistik_penyakit_daerah][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$table->addRow("", "<b>Total</b>", "", $total, round($persen_total));

		$_SESSION[rekmed][statistik_penyakit_daerah][total] = $total;
		$_SESSION[rekmed][statistik_penyakit_daerah][persen_total] = round($persen_total);

		if(empty($_SESSION[rekmed][statistik_penyakit_daerah][jml])) $_SESSION[rekmed][statistik_penyakit_daerah][jml][0] = 1;
		if(empty($_SESSION[rekmed][statistik_penyakit_daerah][kode])) $_SESSION[rekmed][statistik_penyakit_daerah][kode][0] = "No Data";
		$ret = $table->build();
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_penyakit_daerah][title]));
		$_SESSION[rekmed][statistik_penyakit_daerah][graph] = "<img src=\"" . URL . "rekmed/statistik_penyakit_daerah_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Penyakit\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_penyakit_daerah][graph]);
		return $objResponse;
	}

	function list_pasien($hal, $id_icd, $val) {
		unset($_SESSION[rekmed][statistik_penyakit_daerah][list_pasien][cetak]);
		...
		if($val[propinsi_id]) {
			$_q .= " AND prop.id = '".$val[propinsi_id]."' ";
			$ply .= "\nPropinsi " . $_SESSION[rekmed][statistik_penyakit_daerah][prop];

			if($val[kabupaten_id]) {
				$_q .= " AND kab.id = '".$val[kabupaten_id]."' ";
				$ply .= "\nKabupaten " . $_SESSION[rekmed][statistik_penyakit_daerah][kab];
				if($val[kecamatan_id]) {
					$_q .= " AND kec.id = '".$val[kecamatan_id]."' ";
					$ply .= "\nKecamatan " . $_SESSION[rekmed][statistik_penyakit_daerah][kec];
					if($val[desa_id]) {
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$ply .= "\nKelurahan " . $_SESSION[rekmed][statistik_penyakit_daerah][desa];
					}
				}
			}
		}
		...
		$title = "Daftar Pasien Penderita ".$data[0][diagnosa]." [ ".$data[0][kode_icd]."] " . $ply ." \nPeriode " . $tanggal_awal . " s.d. " . $tanggal_akhir;
		$objResponse->addClear("list_pasien", "style.display");
		$atas = "<div style=\"text-align:right;width:100%;\">";
		$atas .= "<img src=\"". IMAGES_URL . "printer.gif\" alt=\"Cetak\" onclick=\"cetak('".URL."rekmed/statistik_penyakit_daerah_list_pasien_cetak/?hal=".$hal."');\" class=\"printer_button\" />";
		...
		$_SESSION[rekmed][statistik_penyakit_daerah][list_pasien][cetak][is_last_page] = $is_last_page;
		$_SESSION[rekmed][statistik_penyakit_daerah][list_pasien][cetak][title] = $title;
		$_SESSION[rekmed][statistik_penyakit_daerah][list_pasien][cetak][content] = $ret;
		$_SESSION[rekmed][statistik_penyakit_daerah][list_pasien][cetak][ket_hal] = $ket_hal;
