...
Class Statistik_Kunjungan_IGD {
	...
	function get_kunjungan($val) {
		unset($_SESSION[rekmed][statistik_kunjungan_igd]);
		...
		for($i=0;$i<=$selisih;$i++) {
			if($val[tampilkan] == "hari") {
				...
				$_SESSION[rekmed][statistik_kunjungan_igd][label_tick][$i] = @date("j", @mktime(1, 1, 1,$tgl_start_bln, ($tgl_start_tgl+$i), $tgl_start_thn));
				...
				$_SESSION[rekmed][statistik_kunjungan_igd][label_tick][$i] = @date("M y", @mktime(1, 1, 1, ($tgl_start_bln+$i), 1, $tgl_start_thn));
				...
				$_SESSION[rekmed][statistik_kunjungan_igd][label_tick][$i] = @date("Y", @mktime(1, 1, 1, 1, 1, ($tgl_start_thn+$i)));
				$label_x[$i] = "Tahun";
			...
			$_SESSION[rekmed][statistik_kunjungan_igd][jml_baru][$i] = $baru[$i][jml_baru];
			$_SESSION[rekmed][statistik_kunjungan_igd][jml_lama][$i] = $lama[$i][jml_lama];
			$_SESSION[rekmed][statistik_kunjungan_igd][tgl_periksa][$i] = $tgl_periksa[$i];
		}
		//$objResponse->addAlert(print_r($lama));
		$_SESSION[rekmed][statistik_kunjungan_igd][title] = $title . "\nPeriode " . tanggalIndo($tgl_start, "j F Y") . " s.d. " . tanggalIndo($tgl_end, "j F Y");
		$labelx = @array_unique($label_x);
		$_SESSION[rekmed][statistik_kunjungan_igd][label_x] = @implode(", ",$labelx);
		...
		$_SESSION[rekmed][statistik_kunjungan_igd][th_0] = $table->Th[0];
		$_SESSION[rekmed][statistik_kunjungan_igd][th_1] = $table->Th[1];
		$_SESSION[rekmed][statistik_kunjungan_igd][extra_th_0] = $table->extra_th[0];
		$_SESSION[rekmed][statistik_kunjungan_igd][row] = $table->Row[0];
		
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$_SESSION[rekmed][statistik_kunjungan_igd][graph] = "<img src=\"" . URL . "rekmed/statistik_kunjungan_igd_graph_bar/?md5=".md5(date("Ymdhis"))."\" alt=\"Kunjungan\" />";
		$_SESSION[rekmed][statistik_kunjungan_igd][title] = $title;
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_kunjungan_igd][title]));
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_kunjungan_igd][graph]);
		return $objResponse;
	}
