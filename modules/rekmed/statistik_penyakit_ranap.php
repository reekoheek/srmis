<?
$_TITLE = "Statistik Penyakit Rawat Inap";
Class Statistik_Penyakit {
	...
	function list_pasien($hal, $id_icd, $val) {
		unset($_SESSION[rekmed][statistik_penyakit_ranap][list_pasien][cetak]);
		...
		$title = "Daftar Pasien Rawat Inap".$pelayanan."Penderita ".$data[0][diagnosa]." [ ".$data[0][kode_icd]."] " . $ply ." \nPeriode " . $tanggal_awal . " s.d. " . $tanggal_akhir;
		$objResponse->addClear("list_pasien", "style.display");
		$atas = "<div style=\"text-align:right;width:100%;\">";
		$atas .= "<img src=\"". IMAGES_URL . "printer.gif\" alt=\"Cetak\" onclick=\"cetak('".URL."rekmed/statistik_penyakit_ranap_list_pasien_cetak/?hal=".$hal."');\" class=\"printer_button\" />";
		...
		$_SESSION[rekmed][statistik_penyakit_ranap][list_pasien][cetak][is_last_page] = $is_last_page;
		$_SESSION[rekmed][statistik_penyakit_ranap][list_pasien][cetak][title] = $title;
		$_SESSION[rekmed][statistik_penyakit_ranap][list_pasien][cetak][content] = $ret;
		$_SESSION[rekmed][statistik_penyakit_ranap][list_pasien][cetak][ket_hal] = $ket_hal;
