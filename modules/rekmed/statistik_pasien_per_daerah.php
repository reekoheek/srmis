...
Class Statistik_Pasien_Semua_Pekerjaan {
	...
	function get_pasien($val) {
		...
		$title = "Distribusi Pasien Menurut Daerah";
		
		unset($_SESSION[rekmed][statistik_pasien_per_daerah]);
		$kon = new Konek;
		if($val[propinsi_id]) {
			$kon->sql = "SELECT nama FROM ref_propinsi WHERE id = '".$val[propinsi_id]."'";
			$kon->execute();
			$prop = $kon->getOne();
			$_SESSION[rekmed][statistik_pasien_per_daerah][prop] = $prop[nama];
			$_q .= " AND prop.id = '".$val[propinsi_id]."' ";
			$title .= "\nPropinsi " . $prop[nama];
			$select = "kab.id as id, kab.nama as nama";
			$group_by = "kab.id";

			if($val[kabupaten_id]) {
				$kon->sql = "SELECT nama FROM ref_kabupaten WHERE id = '".$val[kabupaten_id]."'";
				$kon->execute();
				$kab = $kon->getOne();
				$_SESSION[rekmed][statistik_pasien_per_daerah][kab] = $kab[nama];
				$_q .= " AND kab.id = '".$val[kabupaten_id]."' ";
				$title .= "\nKabupaten " . $kab[nama];
				$select = "kec.id as id, kec.nama as nama";
				$group_by = "kec.id";
				if($val[kecamatan_id]) {
					$kon->sql = "SELECT nama FROM ref_kecamatan WHERE id = '".$val[kecamatan_id]."'";
					$kon->execute();
					$kec = $kon->getOne();
					$_SESSION[rekmed][statistik_pasien_per_daerah][kec] = $kec[nama];
					$_q .= " AND kec.id = '".$val[kecamatan_id]."' ";
					$title .= "\nKecamatan " . $kec[nama];
					$select = "des.id as id, des.nama as nama";
					$group_by = "des.id";
					if($val[desa_id]) {
						$kon->sql = "SELECT nama FROM ref_desa WHERE id = '".$val[desa_id]."'";
						$kon->execute();
						$desa = $kon->getOne();
						$_SESSION[rekmed][statistik_pasien_per_daerah][desa] = $desa[nama];
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$title .= "\nKelurahan " . $desa[nama];
						$select = "des.id as id, des.nama as nama";
						$group_by = "des.id";
					} else unset($_SESSION[rekmed][statistik_pasien_per_daerah][desa]);
				} else unset($_SESSION[rekmed][statistik_pasien_per_daerah][kec]);
			} else unset($_SESSION[rekmed][statistik_pasien_per_daerah][kab]);
		} else {
			$select = "prop.id as id, prop.nama as nama";
			$group_by = "prop.id";
			unset($_SESSION[rekmed][statistik_pasien_per_daerah][prop]);
		}
		...
		$_SESSION[rekmed][statistik_pasien_per_daerah][title] = $title;
		$sql = "
			SELECT
				$select, 
				COUNT(p.id) as jml
		...
		for($i=0;$i<sizeof($data);$i++) {
			...
			$table->addOnclickTd(
				"xajax_list_pasien('0', '".$data[$i][id]."', '".addslashes($data[$i][nama])."', xajax.getFormValues('statistik_pasien_per_daerah'))", 
				"xajax_list_pasien('0', '".$data[$i][id]."', '".addslashes($data[$i][nama])."', xajax.getFormValues('statistik_pasien_per_daerah'))", 
				"xajax_list_pasien('0', '".$data[$i][id]."', '".addslashes($data[$i][nama])."', xajax.getFormValues('statistik_pasien_per_daerah'))", 
				"xajax_list_pasien('0', '".$data[$i][id]."', '".addslashes($data[$i][nama])."', xajax.getFormValues('statistik_pasien_per_daerah'))");

			$_SESSION[rekmed][statistik_pasien_per_daerah][no][$i] = ($i+1);
			$_SESSION[rekmed][statistik_pasien_per_daerah][nama][$i] = $data[$i][nama];
			$_SESSION[rekmed][statistik_pasien_per_daerah][jml][$i] = $data[$i][jml];
			$_SESSION[rekmed][statistik_pasien_per_daerah][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$_SESSION[rekmed][statistik_pasien_per_daerah][total] = $total;
		$_SESSION[rekmed][statistik_pasien_per_daerah][persen_total] = round($persen_total);
		$table->addRow("", "<b>Total</b>", $_SESSION[rekmed][statistik_pasien_per_daerah][total], $_SESSION[rekmed][statistik_pasien_per_daerah][persen_total]);
		$ret = $table->build();
		if(empty($_SESSION[rekmed][statistik_pasien_per_daerah][jml])) $_SESSION[rekmed][statistik_pasien_per_daerah][jml][0] = 1;
		if(empty($_SESSION[rekmed][statistik_pasien_per_daerah][nama]))	$_SESSION[rekmed][statistik_pasien_per_daerah][nama][0] = "No Data";
		

		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_pasien_per_daerah][title]));
		$_SESSION[rekmed][statistik_pasien_per_daerah][graph] = "<img src=\"" . URL . "rekmed/statistik_pasien_per_daerah_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Pasien\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_pasien_per_daerah][graph]);
		return $objResponse;
	}

	function list_pasien($hal, $id_daerah, $nama_daerah, $val) {
		unset($_SESSION[rekmed][statistik_pasien_per_daerah][list_pasien]);
		...
		if($val[propinsi_id]) {
			$_q .= " AND prop.id = '".$val[propinsi_id]."' ";
			$title .= "\nPropinsi " . $_SESSION[rekmed][statistik_pasien_per_daerah][prop];
			if($val[kabupaten_id]) {
				$_q .= " AND kab.id = '".$val[kabupaten_id]."' ";
				$title .= "\nKabupaten " . $_SESSION[rekmed][statistik_pasien_per_daerah][kab];
				if($val[kecamatan_id]) {
					$_q .= " AND kec.id = '".$val[kecamatan_id]."' ";
					$title .= "\nKecamatan " . $_SESSION[rekmed][statistik_pasien_per_daerah][kec];
					if($val[desa_id]) {
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$title .= "\nKelurahan " . $_SESSION[rekmed][statistik_pasien_per_daerah][desa];
					} else {
						$_q .= "AND des.id = '".$id_daerah."'";
						$title .= "\nKelurahan " . $nama_daerah;
					}
				} else {
					$_q .= "AND kec.id = '".$id_daerah."'";
					$title .= "\nKecamatan " . $nama_daerah;
				}
			} else {
				$_q .= "AND kab.id = '".$id_daerah."'";
				$title .= "\nKabupaten " . $nama_daerah;
			}
		} else {
			$_q .= "AND prop.id = '".$id_daerah."'";
			$title .= "\nPropinsi " . $nama_daerah;
		}
		...
		$paging->setOnclickValue($id_pekerjaan, "xajax.getFormValues('statistik_pasien_per_daerah')");
		...
		$title = "Daftar Pasien Pasien Menurut Daerah ".$title. "\nPeriode " . $tanggal_awal . " s.d. " . $tanggal_akhir;
		$objResponse->addClear("list_pasien", "style.display");
		$atas = "<div style=\"text-align:right;width:100%;\">";
		$atas .= "<img src=\"". IMAGES_URL . "printer.gif\" alt=\"Cetak\" onclick=\"cetak('".URL."rekmed/statistik_pasien_per_daerah_list_pasien_cetak/?hal=".$hal."');\" class=\"printer_button\" />";
		...
		$_SESSION[rekmed][statistik_pasien_per_daerah][list_pasien][is_last_page] = $is_last_page;
		$_SESSION[rekmed][statistik_pasien_per_daerah][list_pasien][title] = $title;
		$_SESSION[rekmed][statistik_pasien_per_daerah][list_pasien][content] = $ret;
		$_SESSION[rekmed][statistik_pasien_per_daerah][list_pasien][ket_hal] = $ket_hal;
...
$kon->sql = "SELECT id, nama FROM ref_propinsi ORDER BY nama";
$kon->execute();
$data_propinsi = $kon->getAll();

$_xajax->registerFunction(array("get_pasien_check", "Statistik_Pasien_Semua_Pekerjaan", "get_pasien_check"));
$_xajax->registerFunction(array("get_pasien", "Statistik_Pasien_Semua_Pekerjaan", "get_pasien"));
$_xajax->registerFunction(array("list_pasien", "Statistik_Pasien_Semua_Pekerjaan", "list_pasien"));
