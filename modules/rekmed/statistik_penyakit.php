...
Class Statistik_Penyakit {
	...
	function get_penyakit($val) {
		...
		unset($_SESSION[rekmed][statistik_penyakit]);

		$kon = new Konek;
		if($val[jenis]) {
			$_SESSION[rekmed][statistik_penyakit][pjnama] = $val[jenis];
			$_q .= " AND pel.jenis = '".$val[jenis]."' ";
			$title .= "\nPelayanan " . $_SESSION[rekmed][statistik_penyakit][pjnama];
			if($val[pelayanan_id]) {
				$kon->sql = "SELECT nama FROM pelayanan WHERE id = '".$val[pelayanan_id]."'";
				$kon->execute();
				$pel = $kon->getOne();
				$_SESSION[rekmed][statistik_penyakit][pelnama] = $pel[nama];
				$_q .= " AND pel.id = '".$val[pelayanan_id]."' ";
				$title .= " - " . $_SESSION[rekmed][statistik_penyakit][pelnama];
			}
		}
		if($val[propinsi_id]) {
			...
			$kon->execute();
			$prop = $kon->getOne();
			$_SESSION[rekmed][statistik_penyakit][prop] = $prop[nama];
			...
			if($val[kabupaten_id]) {
				...
				$kon->execute();
				$kab = $kon->getOne();
				$_SESSION[rekmed][statistik_penyakit][kab] = $kab[nama];
				...
				if($val[kecamatan_id]) {
					...
					$kon->execute();
					$kec = $kon->getOne();
					$_SESSION[rekmed][statistik_penyakit][kec] = $kec[nama];
					...
					if($val[desa_id]) {
						...
						$kon->execute();
						$desa = $kon->getOne();
						$_SESSION[rekmed][statistik_penyakit][desa] = $desa[nama];
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$title .= "\nKelurahan " . $desa[nama];
					} else unset($_SESSION[rekmed][statistik_penyakit][desa]);
				} else unset($_SESSION[rekmed][statistik_penyakit][kec]);
			} else unset($_SESSION[rekmed][statistik_penyakit][kab]);
		} else unset($_SESSION[rekmed][statistik_penyakit][prop]);
		...
		$_SESSION[rekmed][statistik_penyakit][title] = $title;
		...
		for($i=0;$i<sizeof($new_data);$i++) {
			...
			$_SESSION[rekmed][statistik_penyakit][no][$i] = ($i+1);
			$_SESSION[rekmed][statistik_penyakit][kode][$i] = $new_data[$i][kode_icd];
			$_SESSION[rekmed][statistik_penyakit][nama][$i] = $new_data[$i][nama];
			$_SESSION[rekmed][statistik_penyakit][jml][$i] = $new_data[$i][jml];
			$_SESSION[rekmed][statistik_penyakit][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$table->addRow("", "<b>Total</b>", "", $total, round($persen_total));

		$_SESSION[rekmed][statistik_penyakit][total] = $total;
		$_SESSION[rekmed][statistik_penyakit][persen_total] = round($persen_total);

		if(empty($_SESSION[rekmed][statistik_penyakit][jml])) $_SESSION[rekmed][statistik_penyakit][jml][0] = 1;
		if(empty($_SESSION[rekmed][statistik_penyakit][kode])) $_SESSION[rekmed][statistik_penyakit][kode][0] = "No Data";
		$ret = $table->build();
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_penyakit][title]));
		$_SESSION[rekmed][statistik_penyakit][graph] = "<img src=\"" . URL . "rekmed/statistik_penyakit_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Penyakit\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_penyakit][graph]);
		return $objResponse;
	}

	function list_pasien($hal, $id_icd, $val) {
		unset($_SESSION[statistik][penyakit][list_pasien][cetak]);
		...
		if($val[jenis]) {
			...
			if($val[pelayanan_id]) {
				$_q .= " AND pel.id = '".$val[pelayanan_id]."' ";
				$ply .= " - " . $_SESSION[rekmed][statistik_penyakit][pelnama];
			}
		}
		if($val[propinsi_id]) {
			$_q .= " AND prop.id = '".$val[propinsi_id]."' ";
			$ply .= "\nPropinsi " . $_SESSION[rekmed][statistik_penyakit][prop];

			if($val[kabupaten_id]) {
				$_q .= " AND kab.id = '".$val[kabupaten_id]."' ";
				$ply .= "\nKabupaten " . $_SESSION[rekmed][statistik_penyakit][kab];
				if($val[kecamatan_id]) {
					$_q .= " AND kec.id = '".$val[kecamatan_id]."' ";
					$ply .= "\nKecamatan " . $_SESSION[rekmed][statistik_penyakit][kec];
					if($val[desa_id]) {
						$_q .= " AND des.id = '".$val[desa_id]."' ";
						$ply .= "\nKelurahan " . $_SESSION[rekmed][statistik_penyakit][desa];
					}
				}
			}
		}
		...
		$atas .= "<img src=\"". IMAGES_URL . "printer.gif\" alt=\"Cetak\" onclick=\"cetak('".URL."rekmed/statistik_penyakit_list_pasien_cetak/?hal=".$hal."');\" class=\"printer_button\" />";
		...
		$_SESSION[statistik][penyakit][list_pasien][cetak][is_last_page] = $is_last_page;
		$_SESSION[statistik][penyakit][list_pasien][cetak][title] = $title;
		$_SESSION[statistik][penyakit][list_pasien][cetak][content] = $ret;
		$_SESSION[statistik][penyakit][list_pasien][cetak][ket_hal] = $ket_hal;
