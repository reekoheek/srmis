...
Class Statistik_Kunjungan_Semua_Cara_Masuk {
	...
	function get_pasien($val) {
		...
		$title = "Statistik Kunjungan Berdasar Cara Masuk";
		
		unset($_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk]);
		...
		$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][title] = $title;
		$sql = "
			SELECT
		...
		for($i=0;$i<sizeof($data);$i++) {
			...
			$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][no][$i] = ($i+1);
			$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][nama][$i] = $data[$i][nama];
			$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][jml][$i] = $data[$i][jml];
			$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][total] = $total;
		$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][persen_total] = round($persen_total);
		$table->addRow("", "<b>Total</b>", $_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][total], $_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][persen_total]);
		$ret = $table->build();
		if(empty($_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][jml])) $_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][jml][0] = 1;
		if(empty($_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][nama]))	$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][nama][0] = "No Data";
		

		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][title]));
		$_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][graph] = "<img src=\"" . URL . "rekmed/statistik_kunjungan_semua_cara_masuk_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Pasien\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_kunjungan_semua_cara_masuk][graph]);
		return $objResponse;
	}
