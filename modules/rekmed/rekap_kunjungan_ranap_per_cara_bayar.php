...
Class Lap_Jml_Kunjungan_Per_Pelayanan {
	...
	function get_kunjungan($val) {
		unset($_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar]);
		...
		$title = "Rekapitulasi Jumlah Cara Pembayaran di Unit Rawat Inap";
		...
		for($i=0;$i<=$selisih;$i++) {
			if($val[tampilkan] == "hari") {
				...
				$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][label_tick][$i] = @date("j", @mktime(1, 1, 1,$tgl_start_bln, ($tgl_start_tgl+$i), $tgl_start_thn));
				...
				$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][label_tick][$i] = @date("M y", @mktime(1, 1, 1, ($tgl_start_bln+$i), 1, $tgl_start_thn));
				...
				$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][label_tick][$i] = @date("Y", @mktime(1, 1, 1, 1, 1, ($tgl_start_thn+$i)));
				$label_x[$i] = "Tahun";
		...
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][title] = $title . "\nPeriode " . tanggalIndo($tgl_start, "j F Y") . " s.d. " . tanggalIndo($tgl_end, "j F Y");
		$labelx = @array_unique($label_x);
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][label_x] = @implode(", ",$labelx);
		...
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][th_0] = $table->Th[0];
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][th_1] = $table->Th[1];
		$cb = array('UMUM','JAMSOSTEK','DANA REKSA DESA','KONTRAK','LAIN-LAIN','ASKES');
		
		for($i=0;$i<sizeof($cb);$i++) {
			$table->Row[$i][] = $cb[$i];
			$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][cb][$i] = $cb[$i];
			for($s=0;$s<=$selisih;$s++) {
				if(!$new[jml][$cb[$i]][$s]) $new[jml][$cb[$i]][$s]=0;
				$table->Row[$i][] = $new[jml][$cb[$i]][$s];
				$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][jml][$i][] = $new[jml][$cb[$i]][$s];
			}
			$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][row][$i] = $table->Row[$i];
		}
		//$objResponse->addAlert(print_r($baru));

		$ret = $table->build();
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][graph] = "<img src=\"" . URL . "rekmed/rekap_kunjungan_ranap_per_cara_bayar_graph_line/?md5=".md5(date("Ymdhis"))."\" alt=\"Kunjungan\" />";
		$_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][title] = $title;
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][title]));
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][rekap_kunjungan_ranap_per_cara_bayar][graph]);
		return $objResponse;
	}
