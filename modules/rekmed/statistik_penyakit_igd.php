...
Class Statistik_Penyakit {
	...
	function get_penyakit($val) {
		...
		if($val[tampilkan] != "Semua") {
			$batas = $val[tampilkan];
			$title = "Statistik ".$val[tampilkan]." Besar Penyakit IRD";
		} else {
			$batas = 0;
			$title = "Statistik Penyakit";
		}

		unset($_SESSION[rekmed][statistik_penyakit_igd]);
		...
		$_SESSION[rekmed][statistik_penyakit_igd][title] = $title;
		...
		for($i=0;$i<sizeof($new_data);$i++) {
			...
			$_SESSION[rekmed][statistik_penyakit_igd][no][$i] = ($i+1);
			$_SESSION[rekmed][statistik_penyakit_igd][kode][$i] = $new_data[$i][kode_icd];
			$_SESSION[rekmed][statistik_penyakit_igd][nama][$i] = $new_data[$i][nama];
			$_SESSION[rekmed][statistik_penyakit_igd][jml][$i] = $new_data[$i][jml];
			$_SESSION[rekmed][statistik_penyakit_igd][persen][$i] = $persen;
			$persen_total += $persen;
		}
		$table->addRow("", "<b>Total</b>", "", $total, round($persen_total));

		$_SESSION[rekmed][statistik_penyakit_igd][total] = $total;
		$_SESSION[rekmed][statistik_penyakit_igd][persen_total] = round($persen_total);

		if(empty($_SESSION[rekmed][statistik_penyakit_igd][jml])) $_SESSION[rekmed][statistik_penyakit_igd][jml][0] = 1;
		if(empty($_SESSION[rekmed][statistik_penyakit_igd][kode])) $_SESSION[rekmed][statistik_penyakit_igd][kode][0] = "No Data";
		$ret = $table->build();
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rekmed][statistik_penyakit_igd][title]));
		$_SESSION[rekmed][statistik_penyakit_igd][graph] = "<img src=\"" . URL . "rekmed/statistik_penyakit_igd_graph_pie/?md5=".md5(date("Ymdhis"))."\" alt=\"Penyakit\" />";
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rekmed][statistik_penyakit_igd][graph]);
		return $objResponse;
	}

	function list_pasien($hal, $id_icd, $val) {
		unset($_SESSION[rekmed][statistik_penyakit_igd][list_pasien][cetak]);
		...
		$title = "Daftar Pasien IGD Penderita ".$data[0][diagnosa]." [ ".$data[0][kode_icd]."] " . $ply ." \nPeriode " . $tanggal_awal . " s.d. " . $tanggal_akhir;
		$objResponse->addClear("list_pasien", "style.display");
		$atas = "<div style=\"text-align:right;width:100%;\">";
		$atas .= "<img src=\"". IMAGES_URL . "printer.gif\" alt=\"Cetak\" onclick=\"cetak('".URL."rekmed/statistik_penyakit_igd_list_pasien_cetak/?hal=".$hal."');\" class=\"printer_button\" />";
		...
		$_SESSION[rekmed][statistik_penyakit_igd][list_pasien][cetak][is_last_page] = $is_last_page;
		$_SESSION[rekmed][statistik_penyakit_igd][list_pasien][cetak][title] = $title;
		$_SESSION[rekmed][statistik_penyakit_igd][list_pasien][cetak][content] = $ret;
		$_SESSION[rekmed][statistik_penyakit_igd][list_pasien][cetak][ket_hal] = $ket_hal;
