<?
$_TITLE = "Laporan Pembagian Jasa Tindakan";
Class Kunjungan {
	...
	function list_data($val) {
		...
		$kon = new Konek;
		$title = "Laporan Pembagian Jasa Tindakan";

		if($val[cara_bayar]) {
			$q .= " AND kk.cara_bayar = '".$val[cara_bayar]."' ";
			$title .= "\nCara Pembayaran : " . $val[cara_bayar];
		}
		if($val[jenis_askes]) {
			$q .= " AND kk.jenis_askes = '".$val[jenis_askes]."' ";
			$title .= " - " . $val[jenis_askes];
		}
		if($val[perusahaan_id]) {
			$q .= " AND kk.perusahaan_id = '".$val[perusahaan_id]."' ";
		...
		$sql = "
			...
			WHERE
				(kb.kunjungan_kendaraan_id IS NOT NULL OR kb.kunjungan_kamar_icopim_id IS NOT NULL)
				$q
			GROUP BY
				kb.nama, hak.id
		";
		...
		$table->addExtraTh("rowspan=\"2\"", "rowspan=\"2\"", "colspan=\"3\"", "colspan=\"5\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"", "rowspan=\"2\"");
		$table->addTh("Pemda", "Adm", "Op", "Pemda", "Adm", "Sdm", "Bang", "Op");
		for($i=0;$i<sizeof($data_karcis);$i++) {
			$table->addRow(
				ucfirst(str_replace("_", " ", $data_karcis[$i][nama])), 
