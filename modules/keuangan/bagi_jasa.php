<?
$_TITLE = "Laporan Pembagian Jasa";
Class Kunjungan {

	function list_data($val) {
		unset($_SESSION[keuangan][pendapatan]);
		//get data karcis dan BHP
		$objResponse = new xajaxResponse;
		$kon = new Konek;
		$title = "Laporan Pembagian Jasa";

		if($val[cara_bayar]) {
			$q .= " AND (kk.cara_bayar = '".$val[cara_bayar]."' OR lk.cara_bayar = '".$val[cara_bayar]."' OR rk.cara_bayar = '".$val[cara_bayar]."') ";
			$title .= "\nCara Pembayaran : " . $val[cara_bayar];
		}
		if($val[jenis_askes]) {
			$q .= " AND (kk.jenis_askes = '".$val[jenis_askes]."' OR lk.jenis_askes = '".$val[jenis_askes]."' OR rk.jenis_askes = '".$val[jenis_askes]."') ";
			$title .= " - " . $val[jenis_askes];
		}
		if($val[perusahaan_id]) {
			$q .= " AND (kk.perusahaan_id = '".$val[perusahaan_id]."' OR lk.perusahaan_id = '".$val[perusahaan_id]."' OR rk.perusahaan_id = '".$val[perusahaan_id]."') ";
			$kon->sql = "SELECT nama FROM ref_perusahaan WHERE id = '".$val[perusahaan_id]."'";
			$perusahaan = $kon->getOne();
			$title .= " - " . $perusahaan[nama];
		}
		if($val[tempat_pembayaran]) {
			$q .= " AND kw.tempat_pembayaran = '".$val[tempat_pembayaran]."' ";
			$title .= "\nTempat Pembayaran : " . $val[tempat_pembayaran];
		}
		...
		$sql = "
			...
			WHERE
				(kb.karcis_id IS NOT NULL OR kb.bhp_id IS NOT NULL OR kb.lab_specimen_id IS NOT NULL OR kb.radio_pemeriksaan_id IS NOT NULL)
				$q
			GROUP BY
				/*nama,nama_hak,bhp_p,mampu_bayar*/
				/*kb.hak_id, kb.karcis_id, kb.bhp_id, kb.lab_specimen_id, kb.radio_pemeriksaan_id*/
				kb.id
			ORDER BY
				kb.hak_id DESC, kb.karcis_id DESC, kb.bhp_id DESC, kb.lab_kunjungan_id DESC, kb.radio_kunjungan_id DESC
		";
		...
		for($i=0;$i<sizeof($data_karcis);$i++) {
			$table->addRow(
				...
				uangIndo($data_karcis[$i][bhp_p], false),
				uangIndo($data_karcis[$i][bhp_rs_adm], false),
				uangIndo($data_karcis[$i][bhp_rs_op], false),
				"?",
				"?",
		...
		$_SESSION[keuangan][pendapatan][title] = $judul;
		$_SESSION[keuangan][pendapatan][content] = $content;
		return $objResponse;
	}
}
