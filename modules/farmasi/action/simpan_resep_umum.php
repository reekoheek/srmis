...
if ($num_cari<1)
{
$sql="insert into resep_head (no_resep,param_no,fld02,created_datetime,created_user,tgl,unit_id,flags,fld03,cara_masuk,no_ket) values('$no_resep','$param_no','$nama_pasien',now(),'$usercreated','$tgl','$unit_id','$flags','$rs_asal','$jenis_ket','$no_ket')";
mysql_query($sql);
}
...
if ($_POST['nama_obat'] == "")
	{
		print "<script>alert('Nama Obat Harus Di Isi Terlebih Dahulu.');location.href='home.php?hal=content/resep_reg_umum&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien&rs_asal=$rs_asal&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	else if ($_POST['ket'] == "")
	{
		print "<script>alert('Keterangan Harus Di Isi Terlebih Dahulu.');location.href='home.php?hal=content/resep_reg_umum&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien&rs_asal=$rs_asal&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	...
	if ($_POST['jumlah'] > $rk['stok'])
	{
		print "<script>alert('MAAF,..!! Jumlah Obat yang diminta lebih besar dari Stok Obat.');location.href='home.php?hal=content/resep_reg_umum&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien&rs_asal=$rs_asal&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>";
	}
	else
	{
	//ini baru rumus
...
print "<script>location.href='home.php?hal=content/resep_reg_umum&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien&rs_asal=$rs_asal&jenis_ket=$jenis_ket&no_ket=$no_ket'</script>"; 
	}
?>
