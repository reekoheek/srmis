...
$sql="insert into resep_head (no_resep,param_no,pasien_id,created_datetime,created_user,tgl,unit_id, cara_masuk,flags) 
	  values('$no_resep','$param_no','$pasien_id',now(),'$usercreated','$tgl','$unit_id', 'RAWAT INAP','3')";
mysql_query($sql);
}
else
{
...
if ($_POST['nama_obat'] == "")
	{
		print "<script>alert('Nama Obat Harus Di Isi Terlebih Dahulu.');location.href='home.php?hal=content/resep_reg_rawat_inap&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien'</script>";
	}
	else if ($_POST['ket'] == "")
	{
		print "<script>alert('Keterangan Harus Di Isi Terlebih Dahulu.');location.href='home.php?hal=content/resep_reg_rawat_inap&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien'</script>";
	}
	...
	{
		print "<script>alert('MAAF,..!! Jumlah Obat yang diminta lebih besar dari Stok Obat.');location.href='home.php?hal=content/resep_reg_rawat_inap&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien'</script>";
	}
	else
	{
	//ini baru rumus
...
print "<script>location.href='home.php?hal=content/resep_reg_rawat_inap&pasien_id=$pasien_id&param_no=$param_no&no_resep=$no_resep&nama=$nama_pasien'</script>"; 
	}
?>
