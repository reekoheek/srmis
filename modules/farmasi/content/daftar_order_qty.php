				...
				<tr>
					...
					<tr valign="top">
						...
						<?php
							...
							if ($cari)
							{
								$query_rec  = mysql_query ("SELECT * FROM quantity,ms_barang WHERE quantity.barang_id = ms_barang.id AND ms_barang.kd_barang LIKE '$cari%' AND ms_barang.flags <> '9' ORDER BY quantity.barang_id ASC");
							}
							else
							{
								$query_rec  = mysql_query ("SELECT * FROM quantity,ms_barang WHERE quantity.barang_id = ms_barang.id AND ms_barang.flags <> '9' ORDER BY quantity.barang_id ASC");
							}
							$rec=0;
						...
						<tr>
							<td>
								<?php
									...
									if ($cari)
									{
									$query  = mysql_query ("SELECT * FROM quantity,ms_barang WHERE quantity.barang_id = ms_barang.id AND ms_barang.kd_barang LIKE '$cari%' AND ms_barang.flags <> '9' ORDER BY quantity.barang_id ASC");
									}
									else
									{
									$query  = mysql_query ("SELECT * FROM quantity,ms_barang WHERE quantity.barang_id = ms_barang.id AND ms_barang.flags <> '9' ORDER BY quantity.barang_id ASC LIMIT $offset, $rowsPerPage");
									}
									...
									echo '<table cellpadding=2 cellspacing=2 width=100% >
											<tr bgcolor=#414141 align=center>
												...
												<td><font color=#FFFFFF width=110px>Stock On Order</font></td>
									...
									while ($result = mysql_fetch_array($query))
									{
										...
										$q_spb = mysql_query("SELECT * FROM detail_spb WHERE LAST_INSERT_ID(barang_id) AND barang_id = '$result[barang_id]' ORDER BY barang_id DESC LIMIT 1");		
										...
										echo "<td width=180px>$result[nama]</td>
											<td align=right width=80px>$result[max_qty]</td>
											<td align=right width=110px>$result[stok]</td>
											<td align=right width=110px>$stock_on_order</td>
									...
									$query   = "SELECT COUNT(quantity.id) AS numrows FROM quantity,ms_barang WHERE quantity.barang_id=ms_barang.id AND ms_barang.flags<>'9' ORDER BY quantity.barang_id ASC";
