 ...
 * @copyright 2011
 * @internal  This is detail container for po_processing.php
 *
 */
?>
        ...
        while ($exQueryDet = mysql_fetch_assoc($temp)){
            ...
            foreach ($toField as $key => $val){
                ...
                print "<td align='center'>
                      ...
                      ";
        ?>
                <input type ='text' maxlength='5' size=8 name='txtSat<?=$no?>' value='<?=$exQueryDet[po_satuan]?>' /></td>                
                <td align='center'><input type ='text' maxlength='4' size=8 name='txtQty<?=$no?>' value='<?=$exQueryDet[po_qty]?>' onblur="document.frmPOCreate.txtSubtotal<?=$no?>.value= parseInt(document.frmPOCreate.txtHarga<?=$no?>.value * document.frmPOCreate.txtQty<?=$no?>.value) ;"/></td>
                <td align='center'><input type ='text' maxlength='9' size=10 name='txtHarga<?=$no?>' value='<?=$exQueryDet[po_harga]?>' onblur="
                document.frmPOCreate.txtSubtotal<?=$no?>.value = parseInt(document.frmPOCreate.txtHarga<?=$no?>.value * document.frmPOCreate.txtQty<?=$no?>.value);
                document.frmPOCreate.txtHTotal.value.toFix(2) += parseInt(document.frmPOCreate.txtSubtotal<?=$no?>.value);" /> </td>
                <td align='center'><input type ='text' maxlength='3' size=5 name='txtDisc<?=$no?>' value='<?=$exQueryDet[po_discount]?>' onblur="document.frmPOCreate.txtADisc<?=$no?>.value=parseInt((document.frmPOCreate.txtDisc<?=$no?>.value /100) * document.frmPOCreate.txtHarga<?=$no?>.value );" /></td>
                <td align='center'><input type ='text' name='txtADisc<?=$no?>' size=15 value='<?=$exQueryDet[po_discammount]?>' onfocus="document.frmPOCreate.txtSubtotal<?=$no?>.value=parseInt(document.frmPOCreate.txtHTotal.value - document.frmPOCreate.txtADisc<?=$no?>.value) ;" /></td>
                <td align='center'><input type ='text' name='txtSubtotal<?=$no?>' size=15 readonly="true" value='<?=$exQueryDet[po_subtotal]?>' />
