				...
				<tr>
					...
					<table border="0" cellpadding="2" cellspacing="2" width="100%">
						<tr>
							<td align="right">
								<div id="container">
								<form method="post" action="home.php?hal=content/daftar_racik_obat_radiologi" enctype="multipart/form-data">
						...
						<tr>
							<td>
								<?php
									...
									barang_unit.unit_id='".$_SESSION['U_UNITID']."' AND ms_barang.kd_barang LIKE '$cari%' ORDER BY ms_barang.ex_year,ms_barang.ex_month,ms_barang.ex_date ASC");
									}
									else
									{
									$query  = mysql_query ("SELECT * FROM ms_barang,barang_unit WHERE barang_unit.flags='1' AND ms_barang.id=barang_unit.barang_id AND 
									...
									while ($result = mysql_fetch_array($query))
									{
										...
										echo "<td width=70px>$result[kd_barang]</td>
											...
											<form method=post action='home.php?hal=content/input_racik_obat_radiologi' enctype=multipart/form-data>
									...
									{
   										$page = $pageNum - 1;
   								   	 	$prev = " <a href=\"$self?page=$page&hal=content/daftar_racik_obat_radiologi&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&laquo;]</font></a> ";

    									$first = " <a href=\"$self?page=1&hal=content/daftar_racik_obat_radiologi&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&laquo;&laquo;]</font></a> ";
									}
									else
									{
   			 							$prev  = ' [&laquo;] ';
									...
									{
    									$page = $pageNum + 1;
    									$next = " <a href=\"$self?page=$page&hal=content/daftar_racik_obat_radiologi&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&raquo;]</font></a> ";

    									$last = " <a href=\"$self?page=$maxPage&hal=content/daftar_racik_obat_radiologi&nama=$nama&ket=$ket&id=$id&no_resep=$no_resep&no_racik=$no_racik&param_no=$param_no&pasien_id=$pasien_id&nama_racikan=$nama_racikan\"><font color='#565957' size=-1>[&raquo;&raquo;]</font></a> ";
									}
									else
									{
   										$next = ' [&raquo;] ';
