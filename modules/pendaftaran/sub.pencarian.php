<?
   class Cari_Pasien {
      ...
      function get_kabupaten($id_propinsi, $id_sel = null) {
         ...
         for($i = 0; $i < sizeof($data); $i++) {
            if($data[$i][id] == $id_sel) {
               $objResponse->addScript("addOption('cari_kabupaten_id','kabupaten_".$data[$i][id].
                  "','".$data[$i][nama]."','".$data[$i][id]."',false,true);");
            } else {
               $objResponse->addScript("addOption('cari_kabupaten_id','kabupaten_".$data[$i][id].
                  "','".$data[$i][nama]."','".$data[$i][id]."',false,false);");
            }
         }
      ...
      function get_kecamatan($id_kabupaten, $id_sel = null) {
         ...
         for($i = 0; $i < sizeof($data); $i++) {
            if($data[$i][id] == $id_sel) {
               $objResponse->addScript("addOption('cari_kecamatan_id','kecamatan_".$data[$i][id].
                  "','".$data[$i][nama]."','".$data[$i][id]."',false,true);");
            } else {
               $objResponse->addScript("addOption('cari_kecamatan_id','kecamatan_".$data[$i][id].
                  "','".$data[$i][nama]."','".$data[$i][id]."',false,false);");
   ...
   $_xajax->registerFunction(array("hapus_pasien_confirm", "Cari_Pasien",
      "hapus_pasien_confirm"));
   $_xajax->registerFunction(array("daftar_dari_cari", "Cari_Pasien",
      "daftar_dari_cari"));
   $_xajax->registerFunction(array("cari_get_kabupaten", "Cari_Pasien",
      "get_kabupaten"));
   $_xajax->registerFunction(array("cari_get_kecamatan", "Cari_Pasien",
      "get_kecamatan"));
