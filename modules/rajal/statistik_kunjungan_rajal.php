...
Class Statistik_Kunjungan_Rawat_Jalan {
	...
	function get_kunjungan($val) {
		unset($_SESSION[rajal][statistik_kunjungan_rajal]);
		...
		$objResponse = new xajaxResponse();
		$kon = new Konek;
		//CEK SESSION, JIKA LOGIN SEBAGAI USER MASING-MASING PELAYANAN RAWAT JALAN
		...
		for($i=0;$i<=$selisih;$i++) {
			if($val[tampilkan] == "hari") {
				...
				$_SESSION[rajal][statistik_kunjungan_rajal][label_tick][$i] = @date("j", @mktime(1, 1, 1,$tgl_start_bln, ($tgl_start_tgl+$i), $tgl_start_thn));
				...
				$_SESSION[rajal][statistik_kunjungan_rajal][label_tick][$i] = @date("M y", @mktime(1, 1, 1, ($tgl_start_bln+$i), 1, $tgl_start_thn));
				...
				$_SESSION[rajal][statistik_kunjungan_rajal][label_tick][$i] = @date("Y", @mktime(1, 1, 1, 1, 1, ($tgl_start_thn+$i)));
				$label_x[$i] = "Tahun";
			...
			$_SESSION[rajal][statistik_kunjungan_rajal][jml_baru][$i] = $baru[$i][jml_baru];
			$_SESSION[rajal][statistik_kunjungan_rajal][jml_lama][$i] = $lama[$i][jml_lama];
			$_SESSION[rajal][statistik_kunjungan_rajal][tgl_periksa][$i] = $tgl_periksa[$i];
		}
		//$objResponse->addAlert(print_r($lama));
		$_SESSION[rajal][statistik_kunjungan_rajal][title] = $title . "\nPeriode " . tanggalIndo($tgl_start, "j F Y") . " s.d. " . tanggalIndo($tgl_end, "j F Y");
		$labelx = @array_unique($label_x);
		$_SESSION[rajal][statistik_kunjungan_rajal][label_x] = @implode(", ",$labelx);
		...
		$_SESSION[rajal][statistik_kunjungan_rajal][th_0] = $table->Th[0];
		$_SESSION[rajal][statistik_kunjungan_rajal][th_1] = $table->Th[1];
		$_SESSION[rajal][statistik_kunjungan_rajal][extra_th_0] = $table->extra_th[0];
		$_SESSION[rajal][statistik_kunjungan_rajal][row] = $table->Row[0];
		
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$_SESSION[rajal][statistik_kunjungan_rajal][graph] = "<img src=\"" . URL . "rajal/statistik_kunjungan_rajal_graph_bar/?md5=".md5(date("Ymdhis"))."\" alt=\"Kunjungan\" />";
		$_SESSION[rajal][statistik_kunjungan_rajal][title] = $title;
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[rajal][statistik_kunjungan_rajal][title]));
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[rajal][statistik_kunjungan_rajal][graph]);
		return $objResponse;
	}
