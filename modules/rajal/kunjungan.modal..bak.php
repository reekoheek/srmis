...
Class Kunjungan_Modal {
	...
	function buka_kunjungan($id_kunjungan_kamar) {
		...
		$sql = "
			SELECT 
				...
				rp.nama as perujuk,
				CONCAT_WS(' - ',kk.cara_bayar, kk.jenis_askes) as cara_bayar
			FROM 
				kunjungan k
		...
		$objResponse->addAssign("input_perujuk", "innerHTML", $data[perujuk]);
		...
		$objResponse->addAssign("input_tgl_periksa", "innerHTML", tanggalIndo($data[tgl_periksa], 'j F Y'));
	...
	function simpan_kunjungan_check($val) {
		...
		if(!$val[input_dokter_id]) {
			...
			$objResponse->addAlert('Silakan pilih Keadaan keluar.');
