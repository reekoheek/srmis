...
Class Kunjungan {
	...
	function list_data($val) {
		unset($_SESSION[radio][pendapatan]);
		...
		$sql = "
			...
			WHERE
				(kb.radio_pemeriksaan_id IS NOT NULL OR kb.bhp_id IS NOT NULL)
				AND kw.tempat_pembayaran = 'RADIO'
				$q
			GROUP BY
				kb.radio_pemeriksaan_id, kb.bhp_id, kb.hak_id
		";
		...
		$_SESSION[radio][pendapatan][title] = $judul;
		$_SESSION[radio][pendapatan][content] = $content;
		return $objResponse;
	}
}
