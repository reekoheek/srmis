<?
$_TITLE = "Statistik Kunjungan Radiologi Berdasar Cara Masuk";
...
Class Statistik_Kunjungan_IGD {
	...
	function get_kunjungan($val) {
		unset($_SESSION[radio][statistik_kunjungan_semua_cara_masuk]);
		...
		for($i=0;$i<=$selisih;$i++) {
			if($val[tampilkan] == "hari") {
				...
				$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][radioel_tick][$i] = @date("j", @mktime(1, 1, 1,$tgl_start_bln, ($tgl_start_tgl+$i), $tgl_start_thn));
				...
				$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][radioel_tick][$i] = @date("M y", @mktime(1, 1, 1, ($tgl_start_bln+$i), 1, $tgl_start_thn));
				...
				$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][radioel_tick][$i] = @date("Y", @mktime(1, 1, 1, 1, 1, ($tgl_start_thn+$i)));
				$radioel_x[$i] = "Tahun";
			...
			$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][jml_igd][$i] = $igd[$i][jml_igd];
			$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][jml_rajal][$i] = $rajal[$i][jml_rajal];
			$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][jml_ranap][$i] = $ranap[$i][jml_ranap];
			$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][jml_luar][$i] = $luar[$i][jml_luar];

			$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][tgl_periksa][$i] = $tgl_periksa[$i];
		}
		//$objResponse->addAlert(print_r($lama));
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][title] = $title . "\nPeriode " . tanggalIndo($tgl_start, "j F Y") . " s.d. " . tanggalIndo($tgl_end, "j F Y");
		$radioelx = @array_unique($radioel_x);
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][radioel_x] = @implode(", ",$radioelx);
		...
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][th_0] = $table->Th[0];
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][th_1] = $table->Th[1];
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][extra_th_0] = $table->extra_th[0];
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][row] = $table->Row[0];
		
		$objResponse->addAssign("list_data", "innerHTML", $ret);
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][graph] = "<img src=\"" . URL . "radio/statistik_kunjungan_semua_cara_masuk_graph_line/?md5=".md5(date("Ymdhis"))."\" alt=\"Kunjungan\" />";
		$_SESSION[radio][statistik_kunjungan_semua_cara_masuk][title] = $title;
		$objResponse->addAssign("title", "innerHTML", nl2br($_SESSION[radio][statistik_kunjungan_semua_cara_masuk][title]));
		$objResponse->addAssign("graph", "innerHTML", $_SESSION[radio][statistik_kunjungan_semua_cara_masuk][graph]);
		return $objResponse;
	}
