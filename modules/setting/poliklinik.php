...
Class Pelayanan {
	...
	function get_pelayanan($id) {
        ...
        $objResponse->addAssign("tarif", "value", $data[tarif]);
	...
	function simpan_pelayanan($value) {
		...
		*/
		...
		if(!$value['id_pelayanan']) {
			$kon->sql = "INSERT INTO pelayanan(jenis, spesialisasi_id, nama, hari_buka,tarif) VALUES ('RAWAT JALAN', '".$value[spesialisasi_id]."', '".$value[nama]."', NULLIF('".$value[hari_buka]."',''),NULLIF('".$value[tarif]."',''))";
			$kon->execute();
			$last_id = $kon->last_id;
			$kon->sql = "INSERT INTO kamar(pelayanan_id, kelas, nama) VALUES ('".$last_id."', 'III', '".$value[nama]."')";
			$kon->execute();
		} else {
			$kon->sql = "UPDATE pelayanan SET spesialisasi_id = '".$value[spesialisasi_id]."', nama = '".$value[nama]."', hari_buka = '".$value[hari_buka]."', tarif = '".$value[tarif]."' WHERE id = '".$value[id_pelayanan]."'";
			$kon->execute();
			$kon->sql = "
			UPDATE 
				kamar kmr 
