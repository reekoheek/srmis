...
Class Langsung_Bayar {
	function buka_langsung_bayar($idlk) {
		unset($_SESSION[lab][langsung_bayar]);
		$kon = new Konek;
		//$kon->debug = 1;
		...
		$id_kwitansi = bikinKwitansi("LAB", $val[lb_mampu_bayar], $status);
        $id_kwitansi = tambahNol($id_kwitansi, 20);
		$objResponse->addClear("modal_lb", "style.display");
		$objResponse->addAssign("lb_id_kunjungan_lab", "value", $idlk);
        $objResponse->addAssign("lb_no_trs", "innerHTML", "LAB-".$id_kwitansi);
		...
		$_SESSION[lab][langsung_bayar][data_px] = $data_pasien;
	...
	function simpan_langsung_bayar($val) {
		$kon = new Konek;
		$objResponse = new xajaxResponse;
		$kon->sql = "SELECT kb.id as id, kb.bayar_bhp as bayar_bhp, kb.bayar_jasa as bayar_jasa FROM kunjungan_bayar kb LEFT JOIN kwitansi_detil kwd ON (kwd.kunjungan_bayar_id = kb.id) WHERE kb.lab_kunjungan_id = '".$val[lb_id_kunjungan_lab]."' AND kwd.kwitansi_id IS NULL";
		$kon->execute();
		$data = $kon->getAll();
		if(!empty($data)) {
			if($val[lb_mampu_bayar] < $val[lb_kurang]) $status = "ANGSUR";
			...
			for($i=0;$i<sizeof($data);$i++) {
				//$kon->sql = "UPDATE kunjungan_bayar SET mampu_bayar_bhp = '".$data[$i][mampu_bayar_bhp]."', mampu_bayar_jasa = '".$data[$i][mampu_bayar_jasa]."', kwitansi_id = '".$id_kwitansi."', tempat_pembayaran = 'LAB' WHERE id = '".$data[$i][id]."'";
				//$kon->execute();
			...
			$id_kwitansi = tambahNol($id_kwitansi, 20);
			$objResponse->addScriptCall("cetak_kwitansi", "LAB-" . $id_kwitansi);
