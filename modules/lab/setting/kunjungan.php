<?
$_TITLE = "Administrasi Data Kunjungan";
Class Kunjungan {
	...
	function list_data($hal=0, $val="") {
		...
		for($i=0;$i<sizeof($data);$i++) {
			$table->addRow(
				...
				$data[$i][kunjungan_ke], 
				$data[$i][no_antrian], 
				$data[$i][jenis_pelayanan], 
				$data[$i][pelayanan], 
				$data[$i][kamar], 
				tanggalIndo($data[$i][tgl_daftar], 'j M Y') . "<br />" . tanggalIndo($data[$i][tgl_daftar], 'H:i:s'), 
				tanggalIndo($data[$i][tgl_periksa], 'j M Y'), 
				tanggalIndo($data[$i][tgl_keluar], 'j M Y') . "<br />" . tanggalIndo($data[$i][tgl_keluar], 'H:i:s'), 
