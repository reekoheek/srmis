<?
$_TITLE = "Daftar Kunjungan Laboratorium";
Class Kunjungan {
	...
	function list_data($hal=0, $val="") {
		...
		if($semua_pasien == "0") $s .= " ";
			...
			$s .= "AND DATE(p.tgl_daftar) BETWEEN '".$tgl_daftar_dari."' AND '".$tgl_daftar_sampai."'";
		}*/
		...
		elseif ($val[cara_masuk_p]=='PASIEN LUAR')
		{
			...
			WHERE
				DATE(lk.tgl_periksa) BETWEEN '".$tgl_periksa_dari."' AND '".$tgl_periksa_sampai."'
				AND lk.cara_masuk = 'PASIEN LUAR'
			ORDER BY 
				lk.id DESC
		";
		}
		...
		/*$sql = "
			...
			FROM 
				kunjungan k
				JOIN pasien p ON (p.id = k.pasien_id)
				JOIN lab_kunjungan lk ON (p.id = lk.pasien_id)
				JOIN kunjungan_kamar kk ON (kk.kunjungan_id = k.id)
				JOIN kamar kmr ON (kmr.id = kk.kamar_id)
				JOIN pelayanan pel ON (pel.id = kmr.pelayanan_id)
				LEFT JOIN dokter d ON (d.id = kk.dokter_id)
			WHERE
				pel.jenis = 'RAWAT INAP' OR pel.jenis = 'RAWAT JALAN' OR pel.jenis = 'IGD'
		 		$s
			ORDER BY 
		...
		$_SESSION[lab_kunjungan][hal] = $hal;
		...
		for($i=0;$i<sizeof($data);$i++) {
			...
			{
				$table->addOnclickTd(
					"xajax_buka_kunjungan('".$data[$i][id_kunjungan_lab]."')",
					"xajax_buka_kunjungan('".$data[$i][id_kunjungan_lab]."')",
					"xajax_buka_kunjungan('".$data[$i][id_kunjungan_lab]."')",
					"xajax_buka_kunjungan('".$data[$i][id_kunjungan_lab]."')",
					"xajax_buka_kunjungan('".$data[$i][id_kunjungan_lab]."')",
...
*/
		...
		$kon->execute();
		$_SESSION[lab][hak] = $kon->getAll();

		$kon->sql = "SELECT * FROM sifat";
		$kon->execute();
		$_SESSION[lab][sifat] = $kon->getAll();
      ...
      function ke_kunjungan($val) {
         ...
         $sql = "
			...
			FROM 
				lab_kunjungan lk
			WHERE
				lk.kunjungan_kamar_id = '".$val[dp_idkk]."'
		 ";
		 $kon->sql = $sql;
         $kon->execute();
         ...
         if (!$d)
         {
      	 ...
      	 $sql = "
            ...
            //for($i=0;$i<sizeof($data);$i++) {
				...
				$data = $kon->getOne();
                //$objResponse->addAssign("dp_no_rm", "innerHTML", $data[no_rm]);
                $id_kunjungan_lab = $data[id];
                //$objResponse->addAlert($id_kunjungan_lab);
			//}
			
         	$objResponse->addScriptCall("tutup_daftar_penunjang");
         	$objResponse->addScriptCall("enable_mainbar");
         	$objResponse->addScriptCall("xajax_buka_kunjungan",$id_kunjungan_lab);
         	return $objResponse;
